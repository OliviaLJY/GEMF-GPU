...
TASKS = [
    {
        "algorithm": "gillespie",
        "command": "~/.conda/envs/gemf/bin/python benchmark.py --algorithm gillespie --N {N} --T_MAX 5 --runs 1000 --device cpu --log_file {LOG_FILE}"
    },
    {
        "algorithm": "tau_node",
        "command": "~/.conda/envs/gemf/bin/python benchmark.py --algorithm tau_node --N {N} --T_MAX 5 --runs 1000 --theta 5 --device cpu --log_file {LOG_FILE}"
    },
    {
        "algorithm": "tau_system",
        "command": "~/.conda/envs/gemf/bin/python benchmark.py --algorithm tau_system --N {N} --T_MAX 5 --runs 1000 --theta 5 --device cpu --log_file {LOG_FILE}"
    },
    {
        "algorithm": "gemfpy",
        "command": "~/.conda/envs/gemf/bin/python benchmark.py --algorithm gemfpy --N {N} --T_MAX 5 --runs 1000 --device cpu --log_file {LOG_FILE}"
    }
]
...
def generate_scripts():
    ...
    master_submit_script_path = os.path.join(OUTPUT_DIR, "submit_all.sh")
    with open(master_submit_script_path, "w") as master_file:
        ...
        for task in TASKS:
            for n in N_VALUES:
                ...
                job_name = f"{algorithm}_{n}"
                
                # Construct log file name, handling special case for discrete_time
                if algorithm == "discrete_time":
                    log_file_name = f"{algorithm}_{n}_50.json"
                elif algorithm in ["tau_node", "tau_system"]:
                    log_file_name = f"{algorithm}_{n}_5.json"
                else:
                    log_file_name = f"{algorithm}_{n}.json"
    ...
    print(f"Navigate to '{OUTPUT_DIR}' and run './submit_all.sh' to submit all jobs.")
    print("----------------------------------------------------")
